  ...
  Stack(
    ...
    makeCard(
      ...
      div(
        ...
        Dropdown.shinyInput("gearFilter", label = "Select Gear Count:", options = unique(mtcars$gear), selected = unique(mtcars$gear)[1]),
        Dropdown.shinyInput("carbFilter", label = "Select Carburetor Count:", options = unique(mtcars$carb), selected = unique(mtcars$carb)[1]),
        CompoundButton.shinyInput("button2", style = list(marginTop = "20px", marginBottom = "28px"), text = "Run Analysis", secondaryText = '', className = "analysis-button")
      ), 
      size = 4, 
